...
def getting_kust_ex_kust_ent() -> list[Any]:
    with sqlite3.connect("DB1.db") as db:
        cursor = db.cursor()
...
def Ist():
    with sqlite3.connect("DB1.db") as db:
        cursor = db.cursor()
        PARES = getting_kust_ex_kust_ent()
        for PARE in PARES:
            kust_ex = PARE[0]
            kust_ent = PARE[1]
...
def clear():
    ...
    with sqlite3.connect("DB1.db") as db:
        ...
        # cursor.execute("DELETE FROM for_handmade_choice")
...
#
# # Копируем данные для ручного определения движения
# with sqlite3.connect("DB.db") as db:
#     cursor = db.cursor()
#     # выбираем кусты котрые выходят в следубщие 45 суток
#     cursor.execute("SELECT kust, end FROM exit_")
#     kust_end = cursor.fetchall()
#     today = date.today()
#     kust_end_handmade = []
#     for elem in kust_end:
#         end = datetime.datetime.strptime(elem[1], '%d-%m-%Y').date()
#         if today + timedelta(45) >= end:
#             kust_end_handmade.append(elem[0])
#         else:
#             break
#     cursor.execute("SELECT * FROM raschet")
#     for KPexKPent in cursor.fetchall():
#         KPex = KPexKPent[0]
#         if KPex in kust_end_handmade:
#             cursor.execute(
#                 "INSERT INTO for_handmade_choice(KPex, KPent, gen_rating, GP_rating, Ist_rating, IInd_rating, RUO_rating, ME_rating, LBT_rating, Comment) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
#                 KPexKPent)
#
...
#         cursor.execute("DELETE FROM raschet WHERE KPent=?", (elem,))
#
#     cursor.execute("SELECT DISTINCT KPex FROM for_handmade_choice")
#     for elem in cursor.fetchall():
#         cursor.execute("DELETE FROM raschet WHERE KPex=?", (elem))
#
# # Определяем движение для оставшихся кустов автоматически
# with sqlite3.connect("DB.db") as db:
#     cursor = db.cursor()
#     cursor.execute("SELECT DISTINCT KPex FROM raschet")
#     for KPex in cursor.fetchall():
#         cursor.execute("SELECT max(gen_rating) FROM raschet WHERE KPex=?", KPex)
#         max_gen_rating = cursor.fetchone()[0]
#         if max_gen_rating == None:
#             cursor.execute("INSERT INTO auto_raschet(KPex, KPent, gen_rating) VALUES(?, ?, ?)",
#                            (KPex[0], 'нет кандидата', ' '))
#         else:
#             values = (KPex[0], max_gen_rating)
#             cursor.execute("SELECT KPex, KPent, gen_rating FROM raschet WHERE KPex=? AND gen_rating=?", values)
#             finish_solution = cursor.fetchone()
#             print(finish_solution)
#             cursor.execute("DELETE FROM raschet WHERE KPent=?", (finish_solution[1],))
#             cursor.execute("INSERT INTO auto_raschet(KPex, KPent, gen_rating) VALUES(?, ?, ?)", finish_solution)
